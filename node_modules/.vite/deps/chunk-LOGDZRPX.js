...
function Ei(t2) {
  let e = 0;
  for (const n3 of t2) e += n3.byteLength;
  const n2 = new Uint8Array(e);
  let i = 0;
  for (const e2 of t2) n2.set(new Uint8Array(e2), i), i += e2.byteLength;
  return n2;
}
function Pi(t2, e = 4) {
  return Math.ceil(t2 / e) * e - t2;
}
function Mi(t2) {
  ...
  const a = s._nextUint32(), r = s._nextUint32(), o = s._nextUint32(), l2 = s._nextUint32(), f = s._nextUint64(), c = s._nextUint64(), U = 3 * Math.max(n2.levelCount, 1) * 8, h = new Ci(t2, Oi.length + i, U, true);
  ...
  p._skip(4);
  const _ = p._nextUint16(), u2 = p._nextUint16(), g2 = p._nextUint16(), x2 = p._nextUint16(), y2 = { vendorId: _, descriptorType: u2, versionNumber: g2, colorModel: p._nextUint8(), colorPrimaries: p._nextUint8(), transferFunction: p._nextUint8(), flags: p._nextUint8(), texelBlockDimension: [p._nextUint8(), p._nextUint8(), p._nextUint8(), p._nextUint8()], bytesPlane: [p._nextUint8(), p._nextUint8(), p._nextUint8(), p._nextUint8(), p._nextUint8(), p._nextUint8(), p._nextUint8(), p._nextUint8()], samples: [] }, b = (x2 / 4 - 6) / 4;
...
function Hi(t2, e = {}) {
  ...
  if (t2.globalData) {
    ...
    n2 = Ei([e2, t2.globalData.endpointsData, t2.globalData.selectorsData, t2.globalData.tablesData, t2.globalData.extendedData]);
  }
  const i = [], s = Object.entries(Ni({}, t2.keyValue, !e.keepWriter && { KTXwriter: "KTX-Parse v1.0.1" }));
  s.sort((t3, e2) => t3[0] > e2[0] ? 1 : -1);
  for (const [t3, e2] of s) {
    const n3 = Ti(t3), s2 = "string" == typeof e2 ? Ei([Ti(e2), Fi]) : e2, a2 = n3.byteLength + 1 + s2.byteLength, r2 = Pi(a2, 4);
    i.push(Ei([new Uint32Array([a2]), n3, Fi, s2, new Uint8Array(r2).fill(0)]));
  }
  const a = Ei(i);
  ...
  for (let t3 = 0; t3 < 8; t3++) l2.setUint8(20 + t3, r.bytesPlane[t3]);
  ...
  const c = Oi.length + 68 + 3 * t2.levels.length * 8, U = c + o.byteLength;
  let h = n2.byteLength > 0 ? U + a.byteLength : 0;
  h % 8 && (h += 8 - h % 8);
  const p = [], _ = new DataView(new ArrayBuffer(3 * t2.levels.length * 8)), u2 = new Uint32Array(t2.levels.length);
  let g2 = 0;
  0 === t2.supercompressionScheme && (g2 = (function(t3) {
    const e2 = Math.max(t3, 4), n3 = Math.min(t3, 4);
    let i2 = e2;
    for (; i2 % n3 !== 0; ) i2 += e2;
    return i2;
  })((function(t3) {
    return t3.levels[0].levelData.byteLength / (function(t4) {
      let e2 = 1;
      const n3 = [t4.pixelWidth, t4.pixelHeight, t4.pixelDepth], i2 = (function(t5) {
        const [e3, n4, i3] = t5.dataFormatDescriptor[0].texelBlockDimension;
        return [e3 + 1, n4 + 1, i3 + 1];
      })(t4);
      for (let t5 = 0; t5 < 3; t5++) if (n3[t5] > 0) {
        const s2 = Math.ceil(Math.floor(1 * n3[t5]) / i2[t5]);
        e2 *= Math.max(1, s2);
      }
      return t4.layerCount > 0 && (e2 *= t4.layerCount), t4.faceCount > 0 && (e2 *= t4.faceCount), e2;
    })(t3);
  })(t2)));
  let x2 = (h || U + a.byteLength) + n2.byteLength;
  for (let e2 = t2.levels.length - 1; e2 >= 0; e2--) {
    if (x2 % g2) {
      const t3 = Pi(x2, g2);
      p.push(new Uint8Array(t3)), x2 += t3;
    }
    const n3 = t2.levels[e2];
    p.push(n3.levelData), u2[e2] = x2, x2 += n3.levelData.byteLength;
  }
  for (let e2 = 0; e2 < t2.levels.length; e2++) {
    const n3 = t2.levels[e2];
    _.setBigUint64(24 * e2 + 0, BigInt(u2[e2]), true), _.setBigUint64(24 * e2 + 8, BigInt(n3.levelData.byteLength), true), _.setBigUint64(24 * e2 + 16, BigInt(n3.uncompressedByteLength), true);
  }
  const y2 = new ArrayBuffer(68), b = new DataView(y2);
  return b.setUint32(0, t2.vkFormat, true), b.setUint32(4, t2.typeSize, true), b.setUint32(8, t2.pixelWidth, true), b.setUint32(12, t2.pixelHeight, true), b.setUint32(16, t2.pixelDepth, true), b.setUint32(20, t2.layerCount, true), b.setUint32(24, t2.faceCount, true), b.setUint32(28, t2.levelCount, true), b.setUint32(32, t2.supercompressionScheme, true), b.setUint32(36, c, true), b.setUint32(40, o.byteLength, true), b.setUint32(44, U, true), b.setUint32(48, a.byteLength, true), b.setBigUint64(52, BigInt(n2.byteLength > 0 ? h : 0), true), b.setBigUint64(60, BigInt(n2.byteLength), true), new Uint8Array(Ei([new Uint8Array(Oi).buffer, y2, _.buffer, o, a, h > 0 ? new ArrayBuffer(h - (U + a.byteLength)) : new ArrayBuffer(0), n2, ...p]));
}
...
//# sourceMappingURL=chunk-LOGDZRPX.js.map
